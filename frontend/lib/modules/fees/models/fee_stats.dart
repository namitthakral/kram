...
class OverdueFee {
  ...
  factory OverdueFee.fromJson(Map<String, dynamic> json) {
    return OverdueFee(
      studentFeeId: int.tryParse((json['student_fee_id'] ?? json['studentFeeId'] ?? 0).toString()) ?? 0,
      studentId: int.tryParse((json['student_id'] ?? json['studentId'] ?? 0).toString()) ?? 0,
      ...
      totalOverdueAmount: double.tryParse((json['total_overdue_amount'] ?? json['totalOverdueAmount'] ?? 0).toString()) ?? 0,
      dueDate: DateTime.tryParse((json['due_date'] ?? json['dueDate'])?.toString() ?? '') ?? DateTime.now(),
      daysOverdue: int.tryParse((json['days_overdue'] ?? json['daysOverdue'] ?? 0).toString()) ?? 0,
...
class OverdueSummary {
  ...
  factory OverdueSummary.fromJson(Map<String, dynamic> json) {
    ...
    return OverdueSummary(
      ...
      totalOverdue: double.tryParse((json['totalOverdue'] ?? 0).toString()) ?? 0,
      fees: feesList.map((e) => OverdueFee.fromJson(e is Map<String, dynamic> ? e : Map<String, dynamic>.from(e as Map))).toList(),
    );
  }
}
...
class PaymentSummary {
  ...
  factory PaymentSummary.fromJson(Map<String, dynamic> json) {
    ...
    return PaymentSummary(
      totalPayments: int.tryParse((json['totalPayments'] ?? json['total_payments'] ?? 0).toString()) ?? 0,
      totalAmount: double.tryParse((json['totalAmount'] ?? json['total_amount'] ?? 0).toString()) ?? 0,
      byMethod: json['byMethod'] is Map ? Map<String, dynamic>.from(json['byMethod'] as Map) : {},
      byMode: json['byMode'] is Map ? Map<String, dynamic>.from(json['byMode'] as Map) : {},
      monthlyTrends: monthly is List ? monthly.map((e) => e is Map ? Map<String, dynamic>.from(e) : <String, dynamic>{}).toList() : [],
    );
  }
}
