...
class StudentFee {
  ...
  factory StudentFee.fromJson(Map<String, dynamic> json) => StudentFee(
    ...
    amountDue:
        double.tryParse(
          (json['amountDue'] ?? json['amount_due'])?.toString() ?? '',
        ) ??
        0,
    amountPaid:
        double.tryParse(
          (json['amountPaid'] ?? json['amount_paid'])?.toString() ?? '',
        ) ??
        0,
    lateFeeApplied:
        double.tryParse(
          (json['lateFeeApplied'] ?? json['late_fee_applied'])?.toString() ??
              '',
        ) ??
        0,
    ...
    feeStructure:
        json['feeStructure'] != null
            ? FeeStructure.fromJson(
              json['feeStructure'] as Map<String, dynamic>,
            )
            : json['fee_structure'] != null
            ? FeeStructure.fromJson(
              json['fee_structure'] as Map<String, dynamic>,
            )
            : null,
    semester:
        json['semester'] is Map<String, dynamic>
  ...
  static int _int(v) {
    if (v == null) throw const FormatException('Expected int but got null');
