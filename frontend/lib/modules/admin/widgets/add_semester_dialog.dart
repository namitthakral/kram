...
class _AddSemesterDialogState extends State<AddSemesterDialog> {
  ...
  Widget build(BuildContext context) => CustomFormDialog(
    ...
    subtitle:
        '${context.translate('create_new_semester_for')} ${widget.academicYearName}',
      ...
      children: [
        ...
        CustomTextField(
          label: context.translate(
            _isSchool ? 'term_number' : 'semester_number',
          ),
          hintText: 'e.g. 1',
  ...
  Future<void> _handleCreate() async {
    if (_nameController.text.isEmpty) {
      showCustomSnackbar(
        message: context.translate(
          _isSchool ? 'enter_term_name' : 'enter_semester_name',
        ),
    ...
    if (_numberController.text.isEmpty) {
      showCustomSnackbar(
        message: context.translate(
          _isSchool ? 'enter_term_number' : 'enter_semester_number',
        ),
        type: SnackbarType.warning,
      );
      return;
    ...
    if (semesterNumber == null) {
      showCustomSnackbar(
        message: context.translate(
          _isSchool ? 'invalid_term_number' : 'invalid_semester_number',
        ),
        type: SnackbarType.warning,
      );
      return;
    ...
    try {
      ...
      if (mounted) {
        ...
        showCustomSnackbar(
          message: context.translate(
            _isSchool ? 'term_created' : 'semester_created',
          ),
