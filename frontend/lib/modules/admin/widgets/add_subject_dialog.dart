              ...
              Row(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.book_rounded, color: AppTheme.blue500, size: 22),
                  ),
                  const SizedBox(width: 12),
                  const Text('Add Subject', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppTheme.slate800)),
                  const Spacer(),
  ...
  Future<void> _createSubject() async {
    ...
    try {
      final data = <String, dynamic>{
        ...
        if (_theoryHoursController.text.trim().isNotEmpty) 'theoryHours': int.parse(_theoryHoursController.text.trim()),
        if (_practicalHoursController.text.trim().isNotEmpty) 'practicalHours': int.parse(_practicalHoursController.text.trim()),
      };
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to create subject: $e'), backgroundColor: Colors.red),
        );
      }
    } finally {
