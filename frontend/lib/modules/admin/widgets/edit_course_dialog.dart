...
class EditCourseDialog extends StatefulWidget {
  const EditCourseDialog({required this.course, super.key});
...
class _EditCourseDialogState extends State<EditCourseDialog> {
  ...
  void _initializeControllers() {
    ...
    _totalSemestersController = TextEditingController(
      text: widget.course['totalSemesters']?.toString() ?? '',
    );
                  ...
                  children: [
                    ...
                    CustomTextField(
                      ...
                      label: context.translate(
                        _isSchool ? 'total_terms' : 'total_semesters',
                      ),
                      hintText: context.translate(
                        _isSchool
                            ? 'enter_total_terms'
                            : 'enter_total_semesters',
                      ),
                      keyboardType: TextInputType.number,
                    ),
                    const SizedBox(height: 16),
  ...
  Future<void> _updateCourse() async {
    ...
    if (_totalSemestersController.text.trim().isNotEmpty) {
      courseData['totalSemesters'] = int.tryParse(
        _totalSemestersController.text.trim(),
      );
    }
