...
export class ProgressUpdaterService {
  ...
  async recalculateStudentProgress(studentId: number): Promise<void> {
    ...
    // Recalculate for each enrolled course
    for (const enrollment of enrollments) {
      if (!enrollment.course.program) continue;

      // Find subjects with matching course code
      const subjects = await this.prisma.subject.findMany({
        where: {
          code: enrollment.course.courseCode,
          institutionId: enrollment.course.program.institutionId,
        },
        select: { id: true },
      });
      ...
      for (const subject of subjects) {
        try {
          ...
          this.logger.error(
            `Failed to recalculate progress for student ${studentId}, subject ${subject.id}`,
            error.stack,
          );
        }
      }
  ...
  async dailyProgressRecalculation(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        'Daily progress recalculation failed',
        error.stack,
      );
    }
  }
  ...
  async weeklyAtRiskIdentification(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        'Weekly at-risk identification failed',
        error.stack,
      );
    }
  }
