...
export class AdminService {
  ...
  async createInstitutionalUser(
    ...
    if (roleName === 'student' && createUserDto.studentData) {
      ...
      await this.createParentRecords(student.id, sd, adminInstitutionId, prisma as any)
    ...
    if (!kramid) {
      ...
      return {
        ...
        data: {
          ...
          role: user.role,
          status: user.status,
          mustChangePassword: UserHelpers.needsPasswordChange(user.status),
  ...
  async unlockAccount(userUuid: string, adminInstitutionId: number | null) {
    ...
    return {
      ...
      data: {
        email: user.email,
        loginAttempts: 0,
        status: user.status === ('LOCKED' as UserStatus) ? ('ACTIVE' as UserStatus) : user.status,
      },
    }
  }
  ...
  private async createParentUser(
    ...
    const kramid = await this.generateKramIdForUser(parentUser.id, adminInstitutionId, 'parent', prisma)
