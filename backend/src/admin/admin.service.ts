...
export class AdminService {
  ...
  async createSemester(dto: CreateSemesterDto, institutionId: number | null) {
    ...
    if (
      institutionId !== null &&
      academicYear.institutionId !== institutionId
    ) {
  ...
  async createInstitutionalUser(
    ...
    return await this.prisma.$transaction(async prisma => {
      ...
      // Create user with PENDING_ACTIVATION status (requires password change to become active)
      ...
      if (roleName === 'student' && createUserDto.studentData) {
        ...
        await prisma.parent.create({
          data: {
            ...
            relation:
              (pd.relation as 'FATHER' | 'MOTHER' | 'GUARDIAN' | 'OTHER') ||
              'GUARDIAN',
      ...
      if (!kramid) {
        const result = await prisma.$queryRawUnsafe<{ kramid: string }[]>(
      ...
      return {
        ...
        data: {
          ...
          mustChangePassword: UserHelpers.needsPasswordChange(
            user.accountStatus
          ),
          statusDescription: UserHelpers.getStatusDescription(
            user.accountStatus
          ),
  ...
  async getUsersStats(institutionId: number | null) {
    ...
    const [
      ...
      this.prisma.user.count({
        where: { ...instWhere, accountStatus: 'ACTIVE' },
      }),
      this.prisma.user.count({
        where: { ...instWhere, accountStatus: 'INACTIVE' },
      }),
      this.prisma.user.count({
        where: { ...instWhere, accountStatus: 'SUSPENDED' },
      }),
  ...
  async getUsersByRole(
    ...
    const { page = 1, limit = 10, search, accountStatus } = query
    const skip = (page - 1) * limit
  ...
  async unlockAccount(userUuid: string, adminInstitutionId: number | null) {
    ...
    await this.prisma.user.update({
      ...
      data: {
        loginAttempts: 0,
        // If user was locked, set to appropriate status
  ...
  private async getTeacherPerformanceData(
    ...
    return performanceData.map(data => ({
      ...
      student: Number(data.student_count),
   ...
   */
  private async checkGradingRestriction(
    institutionId: number,
  ...
  async getIdConfig(institutionId: number, adminInstitutionId: number | null) {
    if (adminInstitutionId !== null && institutionId !== adminInstitutionId) {
      throw new ForbiddenException(
        'Access denied to this institution ID config'
      )
    }
   ...
   */
  async checkGradingRestrictionStatus(
    institutionId: number,
    adminInstitutionId: number | null
  ) {
    if (adminInstitutionId !== null && institutionId !== adminInstitutionId) {
