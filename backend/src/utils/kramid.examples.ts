/**
 * Kram ID Examples and Test Cases
 *
 * This file demonstrates the new Kram ID generation with variable-length
...
export function runExamples() {
  ...
  console.log(
    'Kram ID Generation Examples with Variable-Length Institution Codes'
  )
  console.log('='.repeat(80))
  console.log()
  ...
  INSTITUTION_EXAMPLES.forEach(({ name, expectedCode }) => {
    ...
    // Generate sample Kram IDs for different roles
    const studentId = generateKramId(generatedCode, 'student')
    const teacherId = generateKramId(generatedCode, 'teacher')
    const adminId = generateKramId(generatedCode, 'admin')
  ...
  console.log('='.repeat(80))
  console.log('Kram ID Parsing Examples')
  console.log('='.repeat(80))
  console.log()
  ...
  exampleIds.forEach(id => {
    console.log(`Kram ID: ${id}`)
    console.log(`  Valid: ${isValidKramId(id) ? '✅' : '❌'}`)

    const parsed = parseKramId(id)
    if (parsed) {
      ...
      console.log(`  Role Name: ${getRoleNameFromKramId(id)}`)
    }
    console.log()
  })
  ...
  existingCodes.forEach(code => {
    ...
    const sampleId = generateKramId(code, 'student')
  ...
  console.log('✅ Compact Kram IDs (11-14 characters total)')
  ...
  console.log('✅ Auto-generated by database in production')
  console.log()
}
