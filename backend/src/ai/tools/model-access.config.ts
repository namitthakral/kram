...
async function getAdminInstitutionId(user: UserWithRelations, prisma: PrismaService): Promise<number | null> {
    ...
    const staff = await prisma.staff.findUnique({ where: { userId: user.id }, select: { institutionId: true } })
    if (staff) return staff.institutionId

    // Last resort: look up teacher record
    const teacher = await prisma.teacher.findUnique({ where: { userId: user.id }, select: { institutionId: true } })
...
async function getTeacherStudentIds(user: UserWithRelations, prisma: PrismaService): Promise<number[]> {
...
export const MODEL_ACCESS_CONFIG: Record<string, ModelAccessEntry> = {
    Student: {
        prismaModel: 'student',
        description: 'Query student records including admission, enrollment status, and basic info.',
        allowedRoles: ['teacher', 'admin', 'parent'],
        hiddenFields: ['userId', 'createdAt', 'updatedAt'],
        allowedIncludes: ['user', 'course', 'institution', 'enrollments', 'academicRecords', 'attendanceSummary'],
        filterableFields: ['admissionNumber', 'rollNumber', 'currentSemester', 'currentYear', 'gradeLevel', 'section', 'status', 'studentType'],
        scopeByRole: {
    ...
    Enrollment: {
        ...
        description: 'Query student enrollments in subjects/courses. Shows enrollment status and grades.',
    ...
    AcademicRecord: {
        ...
        description: 'Query academic records with marks, grades, and grade points per subject per semester.',
    ...
    Attendance: {
        ...
        description: 'Query individual attendance records by date, student, and class section.',
        allowedRoles: ['student', 'teacher', 'parent', 'admin'],
    ...
    AttendanceSummary: {
        ...
        description: 'Query attendance summaries showing total/attended/absent classes and attendance percentage per subject.',
        allowedRoles: ['student', 'teacher', 'parent', 'admin'],
        hiddenFields: ['createdAt', 'updatedAt'],
        allowedIncludes: ['student', 'subject', 'semester'],
    ...
    Assignment: {
        ...
        description: 'Query assignments with title, description, due dates, marks, and status.',
    ...
    Submission: {
        ...
        description: 'Query assignment submissions with status, marks, and feedback.',
        ...
        scopeByRole: {
            student: async (user) => ({ studentId: user.student?.id || -1 }),
            teacher: async (user) => ({ assignment: { teacherId: user.teacher?.id || -1 } }),
        },
    },
    ...
    StudentFee: {
        ...
        description: 'Query student fee records with amount due, paid, late fees, and payment status.',
    ...
    FeeStructure: {
        ...
        description: 'Query fee structure definitions with fee types, amounts, and due dates.',
        allowedRoles: ['admin'],
        hiddenFields: ['createdAt', 'updatedAt'],
    ...
    Course: {
        prismaModel: 'course',
        description: 'Query courses (degree programs/streams) with their details and subjects.',
        allowedRoles: ['student', 'teacher', 'parent', 'admin'],
        hiddenFields: ['createdAt', 'updatedAt'],
        allowedIncludes: ['subjects', 'institution'],
        filterableFields: ['name', 'code', 'degreeType', 'status'],
        scopeByRole: {
            student: async (user) => ({ institutionId: user.student?.institutionId || -1 }),
            teacher: async (user) => ({ institutionId: user.teacher?.institutionId || -1 }),
    ...
    Subject: {
        prismaModel: 'subject',
        description: 'Query subjects (individual papers) with code, credits, type, and syllabus.',
        allowedRoles: ['student', 'teacher', 'parent', 'admin'],
        hiddenFields: ['createdAt', 'updatedAt'],
        allowedIncludes: ['course'],
        filterableFields: ['subjectName', 'subjectCode', 'subjectType', 'status', 'courseId'],
        scopeByRole: {
            student: async (user) => ({ course: { institutionId: user.student?.institutionId || -1 } }),
            teacher: async (user) => ({ course: { institutionId: user.teacher?.institutionId || -1 } }),
            parent: async (user, prisma) => {
                const student = await prisma.student.findUnique({
                    where: { id: user.parent?.studentId },
                    select: { institutionId: true },
    ...
    ClassSection: {
        ...
        description: 'Query class sections with teacher, subject, schedule, and enrollment capacity.',
    ...
    Notice: {
        ...
        scopeByRole: {
            student: async (user) => ({ institutionId: user.student?.institutionId || -1, isActive: true }),
            teacher: async (user) => ({ institutionId: user.teacher?.institutionId || -1, isActive: true }),
            parent: async (user, prisma) => {
                const student = await prisma.student.findUnique({
                    where: { id: user.parent?.studentId },
                    select: { institutionId: true },
    ...
    StudentProgress: {
        ...
        description: 'Query student progress summaries with overall grades, scores, and improvement areas.',
        allowedRoles: ['student', 'teacher', 'parent', 'admin'],
        hiddenFields: ['createdAt', 'updatedAt'],
        allowedIncludes: ['student', 'subject', 'semester'],
    ...
    DashboardStats: {
        ...
        description: 'Query dashboard statistics for institution-wide metrics (attendance, performance, fees).',
        allowedRoles: ['admin'],
        hiddenFields: ['createdAt', 'updatedAt'],
        allowedIncludes: [],
    ...
    TimeTable: {
        ...
        description: 'Query timetable entries with day, time slot, subject, teacher, and room.',
        allowedRoles: ['student', 'teacher', 'admin'],
        hiddenFields: ['createdAt', 'updatedAt'],
